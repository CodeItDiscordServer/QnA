...
import {
  ...
  SET_REACT_POST_DETAILS, SELECT_DESELECT_POST, CLEAR_SELECTED_POST_LIST
...
const pageState = (state = initialState.pageState, action) => {
  ...
  switch (action.type) {
    ...
    case SELECT_DESELECT_POST:
      let tempState;  
    if (state.selectedPosts.includes(action.id)) {

        let selectedPostsList = [...state.selectedPosts];
        let indexOfPost = selectedPostsList.indexOf(action.id);
        selectedPostsList.splice(indexOfPost,1)
        // alert(selectedPostsList)
        tempState = Object.assign({},state,{selectedPosts:selectedPostsList})
        console.log(tempState)

      } else {
        tempState = Object.assign({}, state, { selectedPosts: [...state.selectedPosts, action.id] })
      }
      return tempState;

      case CLEAR_SELECTED_POST_LIST:
        
        newState = Object.assign({},state,{selectedPosts:[]});
        return newState;
    default:
      return state;
  }
...
export const SelectedPosts = (state) => state.pageState.selectedPosts